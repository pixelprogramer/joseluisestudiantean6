...
export class ComercialComponent implements OnInit {
  ...
  ngOnInit() {
    this._ElementService.pi_poValidarUsuario('ComercialComponent');
  ...
  listarResultados()
  {
    ...
    this._PantallaService.listarPantallaComercial().subscribe(
      respuesta=>{
  ...
  listarCantidades()
  {
    this._PantallaService.cantidadColegioEstudianteComercial().subscribe(
      respuesta=>{
