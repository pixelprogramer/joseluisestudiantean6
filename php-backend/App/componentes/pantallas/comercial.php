<?php
$app->post('/pantallas/calificacion', function () use ($app) {
    $helper = new helper();
...
ot.description as otdescription, (SELECT GROUP_CONCAT(pro.code) odid FROM order_details ordet INNER JOIN products pro ON ordet.id_product = pro.id WHERE ordet.id_order = ord.id) as od
        ...
        WHERE ord.state IN ('ACTIVO','PROCESO')
ORDER BY `ord`.`id`  ASC";
...
$app->post('/calificacion/datosOrders', function () use ($app) {
    $helper = new helper();
    $sql = "SELECT COUNT(ord.id) countCol FROM orders ord WHERE ord.state IN ('ACTIVO', 'PROCESO')";
    $conexion = new conexMsql();
    $contadorColegio = $conexion->consultaComplejaNorAso($sql);

    $sql = "SELECT SUM(A.countOrder) countEst FROM (
