<?php
$app->post('/pantallas/logistica', function () use ($app) {
    $helper = new helper();
...
ot.description as otdescription, (SELECT GROUP_CONCAT(pro.code) odid FROM order_details ordet INNER JOIN products pro ON ordet.id_product = pro.id WHERE ordet.id_order = ord.id) as od
        ...
        WHERE ord.state IN ('ACTIVO','PROCESO') AND ord.id_order_type in (1,2)
ORDER BY `ord`.`id`  ASC";
...
$app->post('/logistica/datosOrders', function () use ($app) {
    $helper = new helper();
    $idOrderType = "1,2";
    $conexion = new conexMsql();
    $sql = "SELECT COUNT(ord.id) countCol FROM orders ord WHERE ord.id_order_type in (" . $idOrderType . ") AND ord.state IN ('ACTIVO', 'PROCESO')";
    ...
    $sql = "SELECT SUM(A.countOrder) as countEst FROM (
                    ...
                    WHERE ord.id_order_type in (" . $idOrderType . ") AND ord.state IN ('ACTIVO', 'PROCESO')
